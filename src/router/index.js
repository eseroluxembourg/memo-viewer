...
import { loadLanguage, initLocale } from '@/utils/i18n';
...
export const routes = [
  {
    ...
    props: true,
    redirect: (to) => {
      const lang = to.params.lang ? to.params.lang : initLocale;
  ...
  },
  {
    ...
    props: true,
    component: () =>
      import(/* webpackChunkName: "Langs" */ '../views/Langs.vue'),
  },
  {
    path: ':pathMatch(.*)*',
  ...
  routes: [
    {
      ...
      beforeEnter(to, from, next) {
        ...
        if (!langsArray.includes(lang)) {
          ...
          const redirectedPath = newPath.replace(`/${lang}`, `/${initLocale}`);
          next({ path: redirectedPath });
          return;
        }
        loadLanguage(lang);
        next();
