<template>
    ...
    <div class="consequence">
      <router-link
        :to="{
          ...
          params: { cardNum: consequence.to.cardNum, lang: $i18n.locale },
        }"
      >
        <picture>
          <source
          ...
          <img
            ...
            :alt="$t('cards.' + consequence.to.cardNum + '.title')"
            :title="$t('cards.' + consequence.to.cardNum + '.title')"
          />
        </picture>
      </router-link>
      ...
      <p class="consequence-origin">
        <picture>
          ...
          <img
            ...
            :alt="$t('cards.' + consequence.from.cardNum + '.title')"
            :title="$t('cards.' + consequence.from.cardNum + '.title')"
          />
        </picture>
      </p>
    </div>
  ...
  <div
    class="consequence-explanation"
    v-if="
      $t(
        'links.' +
          consequence.from.cardNum +
          '_' +
          consequence.to.cardNum +
          '.explanation'
      ) !==
      'links.' +
        consequence.from.cardNum +
...
export default {
  ...
  computed: {
    imgPathDefaultFrom() {
      return `${process.env.BASE_URL}cards/${this.$i18n.locale}/default/${this.consequence.from.cardNum}-front.jpg`;
    },
    imgPathDefaultTo() {
      return `${process.env.BASE_URL}cards/${this.$i18n.locale}/default/${this.consequence.to.cardNum}-front.jpg`;
    },
    imgPathWebpFrom() {
      return CardsService.getCardImgSrcSet(
        this.$i18n.locale,
        this.consequence.from.cardNum
      );
    },
    imgPathWebpTo() {
      return CardsService.getCardImgSrcSet(
        this.$i18n.locale,
        this.consequence.to.cardNum
      );
    },
  },
  methods: {
